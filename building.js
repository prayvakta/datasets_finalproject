...
function pageSetup() {
    ...
    document.getElementById("searchaddress").addEventListener("keyup", function (){ searchByAddress(this.value);},false);   
    document.getElementById("searchvalue").addEventListener("keyup", function (){ searchByValue(this.value);},false);   
...
function searchByAddress(keyvalue){
    ...
    for(var i=0;i<parsedrecord.length;i++)
    {
        var record=parsedrecord[i];
            //check
            originaladdress=record.originaladdress;//assign
            if(originaladdress!==undefined && originaladdress.startsWith(keyvalue.toUpperCase()))//partial match on string
            {
                output+="<tr><td>";
                output+=record.permitnum;
                output+="</td><td>"
...
function searchByValue(keyvalue){
    ...
    for(var i=0;i<parsedrecord.length;i++)
    {
        var record=parsedrecord[i];
            //check
            estprojectcost=record.estprojectcost;//assign
            if(estprojectcost!==undefined && estprojectcost.startsWith(keyvalue.toUpperCase()))//partial match on string
            {
                output+="<tr><td>";
                output+=record.permitnum;
                output+="</td><td>"
